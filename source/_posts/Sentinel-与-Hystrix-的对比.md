...
由上可以看到 Hystrix 的关注点在于以 `隔离` 和 `熔断` 为主的容错机制，超时或被熔断的调用将会快速失败，并可以提供 `fallback` 机制。
...
[Sentinel 控制台（Dashboard）](https://github.com/alibaba/Sentinel/wiki/%E6%8E%A7%E5%88%B6%E5%8F%B0)提供了机器发现、配置规则、查看实时监控、查看调用链路信息等功能，使得用户可以非常方便地去查看监控和进行配置。
##### 生态
